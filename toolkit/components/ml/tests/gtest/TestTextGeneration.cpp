...
class TextGenerationTest : public mozilla::gtest::ipc::TestUtilityProcess {
 protected:
  // This test asserts the process goes away with its last generator, so opt
  // out of the idle grace period that would otherwise keep it up.
  void SetUp() override {
...
TEST_F(TextGenerationTest, EchoRoundTrip) {
  ...
  RefPtr<TextGenerationParent> generator = reservation->CreateTextGeneration(
      ipc::FileDescriptor(),
