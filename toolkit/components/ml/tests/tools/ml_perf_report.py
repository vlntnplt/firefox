...
"""Summarise ML perftest results as native ONNX vs WASM.

Every ML perf task in CI pins a single ONNX backend through the
MOZ_ML_BACKENDS environment variable (taskcluster/kinds/perftest/*.yml):
`<task>-native` measures the native onnxruntime, `<task>-wasm` the wasm
fallback, and a backend that cannot run fails its own job rather than being
quietly dropped. This report's contract follows from that:

    1. Every scheduled ML job must have reported metrics. A failed job or a
       green job with no data is a finding, and it leads the report.
    2. Every feature must have both a NATIVE and a WASM series on every
       platform that scheduled it. A missing half is a finding, unless the
       variant is deliberately unscheduled -- in which case the reason lives
       as a comment in the kind.yml, and in DECLARED_SKIPS below.

The first section is the verdict on those two rules; the comparison numbers
only come after. Anything this script cannot pair is listed, never assumed.
...
Treeherder logs are cached: pass the same --cache-dir twice and the second run
reuses the download instead of hitting the network. --refresh forces a refetch.
...
# Backend tags used in metric names, from BACKEND_TAGS in head.js. Only the ONNX
# ones take part in the comparison; the rest are recognised so their series are
# reported as skipped rather than landing in the "unrecognised" bucket.
...
# Metric naming produced by runMLPerfTest(): "<FEATURE>-<TAG>-<metric>". The tag
# alternation is what delimits the feature name, which itself contains dashes.
TAGGED_RE = re.compile(
    r"^(?P<feature>.+?)-(?P<tag>%s)-(?P<metric>.+)$"
    % "|".join(re.escape(tag) for tag in ALL_TAGS)
)

# (family, tag) -> reason, for backend variants deliberately not scheduled.
# Keep the reason here AND as a comment next to the exclusion in the kind.yml;
# an entry that is not deliberate belongs in a bug, not in this dict.
DECLARED_SKIPS = {}

# (label, unit, higher_is_better) for the metrics worth naming. Anything else
# still gets reported, with these suffix rules deciding how to read it.
METRIC_INFO = {
...
# Columns of the headline table, as (candidate metrics, header, unit). The first
# candidate a feature reports wins: tests that drive several engines at once
# time their setup as "concurrent-init-latency", which is the same column to a
# reader even though it is a different metric.
SUMMARY_COLUMNS = [
...
# What each measured feature is, as (feature, what this row measures). Metric
# names are whatever the test passed to runMLPerfTest(), which is a code
# identifier and not something to put in front of a reader. Several rows share a
# feature because a test measures the same feature more than one way -- autofill
# times its two models separately and then together -- so the report groups them
# instead of leaving four "Autofill" rows scattered across the table. Keyed
# uppercase, since head.js uppercases some names and not others.
FEATURE_NAMES = {
    ...
    # browser_ml_smart_tab_clustering_perf.js starts its timer before calling
    # generateClusters(), whose stubbed _generateEmbeddings() calls
    # createEngine() -- so the model load lands inside the measurement, and
    # destroyMLEngine() after each one guarantees the next pays it again.
    "AGGLOMERATIVE": (
        "Smart tab groups",
...
# Features whose number does not mean what its row suggests, and why. Printed
# under the table that shows them, because a caveat nobody reads next to the
...
# Treeherder platform slugs are build configuration, not something to head a
# table with. Unknown slugs fall through unchanged rather than being guessed at.
PLATFORM_NAMES = {
...
# Tasks that exercise the engine plumbing rather than a shipping feature: a
# template test over a stand-in model, and a stress test that spins up numbered
# engines. Their numbers say nothing about what a user would feel, and their
# "features" (EXAMPLE, engine3, MULTI) are noise in a table of features, so
# their series are left out of the comparison unless --include-harness-tests
# asks for them. Their jobs still count for the coverage verdict.
HARNESS_FAMILIES = {
...
# Fallback for features that do not report model-run-latency (tab clustering
# measures "5-TABS-latency", say): any latency that is not one of the
# setup-phase ones is the closest thing they have to a run time.
...
class Results:
    ...
    def _ingest_metric(self, label, entry):
        ...
        # Soft-failure metrics from before the per-backend job split. They are
        # not measurements, and treating them as one would fabricate a series.
        ...
        if not match:
            ...
            return
        if match["metric"] == "backend-error":
            return
        job.metrics[match["feature"]][match["tag"]][match["metric"]] = value
    ...
    def features(self, platform, include_harness=None):
        """Merged feature -> tag -> metrics for a platform, plus their source.

        One feature's NATIVE and WASM series now come from two different jobs
        (the -native and -wasm task variants), so the comparison merges every
        job of the platform. Returns (features, sources) where sources maps a
        feature to the task families that reported it.
        """
        if include_harness is None:
            include_harness = self.include_harness
        features = defaultdict(lambda: defaultdict(dict))
        sources = defaultdict(set)
        # Tests are not consistent about casing feature names, so "Autofill-X"
        # and "AUTOFILL-X" are the same feature: the first spelling seen is the
        # key, later casings merge into it.
        canon = {}
...
def ingest_treeherder(results, data):
    ...
    Also records the state of every job in the push, run or not: the coverage
    verdict is about what should have reported, which no artifact can say.
    Metric lines from failing jobs' raw logs are NOT ingested -- a job that
    failed aborted mid-measurement, and partial numbers in a comparison table
    are worse than a hole the verdict already names.
    """
...
# Below this, native lost by more than run-to-run noise would explain, which is
# the thing worth spotting in a long table.
...
def fmt_wasm(value, ratio):
    ...
    One cell rather than a value and a separate ratio column: the ratio is only
    ever read next to the number it qualifies, and three extra columns of it
...
def coverage_problems(results):
    ...
    # Rule 2b, data: both series must exist for every feature. Suppressed for
    # a (platform, family) already reported above -- a failed native job would
    # otherwise repeat here once per feature.
    reported = {
        (platform, family)
...
def render_comparison(results):
    ...
    for platform in results.platforms():
        ...
        # feature -> [(run gain, what it measures, cells)], so the rows a test
        # produces for one feature stay together instead of scattering across
        # the table when it is sorted by gain.
        ...
        for name in sorted(families, key=lambda f: -max(g for g, _, _ in families[f])):
            for i, (_, detail, cells) in enumerate(
                ...
                # Only the first row of a feature is labelled, so a group reads
                # as one block.
...
def render_details(results):
    ...
    Only features with both series: a one-sided list of numbers belongs to
    whatever tracks that platform over time, not to a comparison report.
    """
    body = []
    for platform in results.platforms():
