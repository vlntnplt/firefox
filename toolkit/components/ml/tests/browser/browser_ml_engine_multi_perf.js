...
async function runEngineWithMetrics(
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    for (const [metricName, metricVal] of Object.entries(metrics)) {
      const prefixedMetricName = `${engineConfig.engineId}-${tag}-${metricName}`;
...
async function runConcurrentEngines({ backend, tag }) {
  ...
  try {
    ...
    (combinedJournal[`MULTI-${tag}-total-memory-usage`] =
      combinedJournal[`MULTI-${tag}-total-memory-usage`] || []).push(memUsage);
  } finally {
