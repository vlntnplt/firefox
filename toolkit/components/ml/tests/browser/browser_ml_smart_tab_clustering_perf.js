...
// Keyed by "SINGLE-TAB-<backend tag>-<measurement>", filled in as the tasks
// below run and reported once every backend has been measured.
...
async function runLogisticRegression({ backend, tag }) {
  ...
  pushMetric(
    singleTabMetrics,
    `SINGLE-TAB-${tag}-LOGISTIC-REGRESSION-LATENCY`,
    endTime - startTime
  );
  const titles = similarTabs.map(s => s.label);
  Assert.equal(
...
async function runLogisticRegressionWithDomain({ backend, tag }) {
  ...
  pushMetric(
    singleTabMetrics,
    `SINGLE-TAB-${tag}-LR-WITH-DOMAIN-LATENCY`,
    endTime - startTime
  );
...
add_task(async function test_topic_model() {
  ...
  // Reported once, after every backend has run, so each tagged series is
