...
// One Generate request; runs on TextGenerationChild::mThread.
class TextGenerationChild::Generation {
 public:
  ...
  void Run() {
    MOZ_ASSERT(mChild->mThread->IsOnCurrentThread());
...
void TextGenerationChild::ActorDestroy(ActorDestroyReason aReason) {
  ...
  // process behind it; release it on mThread before the thread goes away.
  RefPtr<TextGenerationChild> self = this;
  MOZ_ALWAYS_SUCCEEDS(mThread->Dispatch(
      NS_NewRunnableFunction("TextGenerationChild::ReleaseBackend",
                             [self] { self->mBackend = nullptr; })));
